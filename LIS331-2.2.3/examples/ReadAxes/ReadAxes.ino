...
#define ACCELEROMETER_ADDR 0x19 // Alternative: 0x18
...
void loop()
{
    float x = 0, y = 0, z = 0; // Raw values. To get them in [g] use `getAxisValuesG()`
    //accelerometer.getXValue(x);
    //accelerometer.getYValue(y);
    //accelerometer.getZValue(z);

    accelerometer.getAxisValuesG(x, y, z);

    // CORRECTING FOR OFFSETS (i.e. x and y vals tend to be ~0.6 higher than they should)
    x -= 0.72;
    y -= 0.62;
